...
export function ComicsPage() {
  ...
  return (
    <div className="relative min-h-screen bg-[#030712]">
          ...
          {COMICS.map((comic, i) => {
            ...
            return (
              <motion.button
                key={comic.id}
                type="button"
                onClick={() => openComic(comic.slug)}
                initial={{ opacity: 0, y: 24 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.45, delay: 0.05 * i }}
                className="glass-card group flex flex-col text-left transition hover:-translate-y-1 hover:border-cyan-400/30"
              >
      ...
      {activeComic && <ComicReader comic={activeComic} onClose={closeReader} />}
    </div>
  );
}
